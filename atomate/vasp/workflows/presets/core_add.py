...
from atomate.vasp.workflows.base.neb import get_wf_neb
...
def wf_neb_spectra(structures, c=None):
    """
    Nudged Elastic Band workflow from the given structures and config dict.

    Args:
        structures (list of Structure): input structures
        c (dict): workflow config dict
    ...
    mip = {"incar_update":{"ENCUT": 520, "EDIFF": 1e-6, "LAECHG":False}}
    wf = add_modify_incar(wf, modify_incar_params=mip)

    wf = add_common_powerups(wf, c)

    if c.get("ADD_WF_METADATA", ADD_WF_METADATA):
        wf = add_wf_metadata(wf, structures)
